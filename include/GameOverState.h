...
private:
    ...
    float m_PlayerCastleCurHP;
    float m_PlayerCastleMaxHP;
    int m_StartCount = 0;
...
public:
    GameOverState(Game *game, bool isWin, float playerCastleCurHP, float playerCastleMaxHP, std::map<std::string, int> &diedKnights, std::map<std::string, int> &diedEnemies);
    void Enter() override;
    void Exit() override;
