...
TextButton::TextButton(SDL_Renderer *rdr, const char *fontPath, std::string txt, float sz, SDL_Color defaultClr,
...
ImageButton::ImageButton(SDL_Renderer *rdr, const char *path1, const char *path2) : renderer(rdr)
{
...
ImageButton::ImageButton(SDL_Renderer *rdr, const char *path1, const char *path2, float w, float h) : renderer(rdr)
{
    defaultTex = IMG_LoadTexture(renderer, path1);
    hoverdTex = IMG_LoadTexture(renderer, path2);
...
{
    SDL_FRect dest = dims.toRect();
    SDL_RenderTexture(renderer, (isHovered ? hoverdTex : defaultTex), nullptr, &dest);
}

void ImageButton::setPos(Vector2 pos)
{
    dims.pos = pos;
}

void ImageButton::setSize(Vector2 size)
{
    dims.size = size;
}

bool ImageButton::getIsHovered()
{
    return isHovered;
}
...
CombinedButton::CombinedButton(SDL_Renderer *rdr, const char *imgNormal, const char *imgHover, const char *fontPath,
    ...
    : renderer(rdr)
{
    ...
    textBtn = new TextButton(renderer, fontPath, text, textSize, textClr, textHoverClr);
}

CombinedButton::CombinedButton(SDL_Renderer *rdr, const char *imgNormal, const char *imgHover, float w, float h,
                               const char *fontPath, std::string text, float textSize, SDL_Color textClr,
...
bool CombinedButton::getIsHovered()
{
    return isHovered;
}
...
Transform CombinedButton::getDims()
{
    ...
    combinedDims.size.y = (imgDims.size.y > txtDims.size.y) ? imgDims.size.y : txtDims.size.y;
