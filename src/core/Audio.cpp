...
void Audio::Init() {
	m_AudioDeviceID =
	    SDL_OpenAudioDevice(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &m_AudioSpec);
	m_mixer = MIX_CreateMixerDevice(m_AudioDeviceID, &m_AudioSpec);
	m_track = MIX_CreateTrack(m_mixer);
	for (const std::string &path : musicPaths) LoadMusic(path.c_str());
}
...
void Audio::Shutdown() {
	...
	for (MIX_Audio *music : m_MusicList) MIX_DestroyAudio(music);
	m_MusicList.clear();
	MIX_Quit();
}