...
double round_3dec(double val) { return round(val * 100.0) / 100.0; }

Vector2::Vector2() { this->x = this->y = 0; }
...
void Vector2::fromAngle(
    double angle) {  // Note that this function takes DEG arguments
	this->x = round_3dec(sin(angle * (M_PI / 180.0)));
	this->y = round_3dec(
	    cos((180 - angle) *
	        (M_PI / 180.0)));  // I honestly have no clue why I have to do
