...
HPBar::HPBar(float &MaxHP, float &CurrentHP, Transform &EntityTransform,
    ...
    : maxHP(MaxHP),
      ...
      width(Width) {
	backgroundRect = {entityTransform.pos.x,
	                  entityTransform.pos.y - verticalOffset, width, height};
...
void HPBar::Update() {
	...
	backgroundRect.x =
	    entityTransform.pos.x + (entityTransform.size.x - width) / 2.0f;
...
void HPBar::Render(SDL_Renderer *renderer) {
	...
	SDL_SetRenderDrawColor(renderer, foregroundColor.r, foregroundColor.g,
