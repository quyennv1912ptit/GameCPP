...
void Game::Init()
{
    ...
    if (!SDL_CreateWindowAndRenderer("GameCPP", Config::GetWindowSize().x, Config::GetWindowSize().y, 0, &m_Window,
                                     &m_Renderer))
    {
    ...
    Background::Get().Init(m_Renderer);
...
void Game::Run()
{
    ...
    while (!m_Quit)
    {
        ...
        LAST = NOW;
        if (curBG != Config::GetBackground())
        {
            m_BGTexture = Background::Get().GetBG();
            curBG = Config::GetBackground();
        }
        ...
        if (!states.empty())
        {
            ...
            SDL_RenderTexture(m_Renderer, m_BGTexture, NULL, &dest);
