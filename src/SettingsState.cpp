...
void SettingsState::Render()
{
    ...
    ImGui::SetNextWindowPos(ImVec2(Config::GetWindowSize().x * 0.5f - Config::GetWindowPadding() -
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding,
                        ImVec2(style.FramePadding.x, (comboHeight - ImGui::GetFontSize()) / 2.0f));
    m_CurrentMusicIndex = Config::GetMusic();
    std::string currentMusic = "MUSIC " + std::to_string(m_CurrentMusicIndex + 1);
    ...
    m_CurrentBackgroundIndex = Config::GetBackground();
    std::string currentBackground = "BACKGROUND " + std::to_string(m_CurrentBackgroundIndex + 1);
    if (ImGui::BeginCombo("##Background", currentBackground.c_str()))
    {
        for (int i = 0; i < Background::Get().GetBGCount(); i++)
        {
            std::string itemName = "BACKGROUND " + std::to_string(i + 1);
            bool isSelected = (i == m_CurrentBackgroundIndex);
            if (ImGui::Selectable(itemName.c_str(), isSelected))
            {
                m_CurrentBackgroundIndex = i;
                Background::Get().UpdateBG(i);
            }
            if (isSelected)
                ImGui::SetItemDefaultFocus();
        }
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding,
                        ImVec2(style.FramePadding.x, (sliderHeight - ImGui::GetFontSize()) / 2.0f));
...
{
    SDL_Log("SettingsState Unloaded\n");
}