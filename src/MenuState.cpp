...
void MenuState::Enter() {
	...
	logoBox = new TextBox(renderer, "resources/fonts/mightysouly.ttf",
	                      "TOWER DEFENSE", 80);
	logoBox->setPos({(float)1280 / 2 - logoBox->getDims().size.x / 2, 100});

	startBtn =
	    new TextButton(renderer, "resources/fonts/mightysouly.ttf", "START", 60,
	                   {255, 255, 255, 255}, {100, 100, 100, 255});
	startBtn->setPos({(float)1280 / 2 - startBtn->getDims().size.x / 2, 250});

	settingBtn =
	    new TextButton(renderer, "resources/fonts/mightysouly.ttf", "SETTINGS",
	                   60, {255, 255, 255, 255}, {100, 100, 100, 255});
	settingBtn->setPos(
	    {(float)1280 / 2 - settingBtn->getDims().size.x / 2, 350});

	exitBtn =
	    new TextButton(renderer, "resources/fonts/mightysouly.ttf", "EXIT", 60,
	                   {255, 255, 255, 255}, {100, 100, 100, 255});
